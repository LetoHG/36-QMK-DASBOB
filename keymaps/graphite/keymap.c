...
// clang-format off
/*
 * Keymaps: dasbob / keymaps/graphite
 * Layers (indices): _GRAPHITE, _ISRT, _COLEMAK_DH, _QWERTY,
 *                   _SPECIAL, _NUMBERS, _NAVIGATION, _FUNCTION,
 *                   _MOUSE, _LAYOUT_SELECTION
 *
 * Layout notes:
 *  - Rows shown left-to-right for each half. Thumbs are listed on the final line
 *    for each layer. Keep clang-format off for visual alignment.
 */
